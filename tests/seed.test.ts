...
import { type ConfigStore, seedDepartmentFromTemplate, cloneWorkflowDef } from "@vc/server";
...
describe("seedDepartmentFromTemplate (Phase 1.6 — engine generik)", () => {
  ...
  it("seed Marketing → department + workflow (cloned) + 1 agent per role", async () => {
    ...
    expect(await store.getWorkflow(seeded.workflow.id)).toBeTruthy();

    // tersimpan di DB & muncul di snapshot, urutan DETERMINISTIK sesuai urutan role template
    const snap = (await store.getWorldSnapshot(c.id))!;
  ...
  it("dua seed dari template sama → workflow id berbeda (tak bentrok)", async () => {
    const c = await store.createCompany({ name: "Co" });
    const f = await store.createFloor(c.id, { name: "L1" });
    const a = await seedDepartmentFromTemplate(store, { companyId: c.id, floorId: f.id, template: MARKETING_TEMPLATE });
    const b = await seedDepartmentFromTemplate(store, { companyId: c.id, floorId: f.id, template: MARKETING_TEMPLATE });
